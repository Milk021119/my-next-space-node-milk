...
const ImageGrid = ({ images }: { images: string[] }) => {
  ...
  if (validImages.length === 1) return <div className="mt-3 relative w-full max-w-[80%] aspect-video rounded-lg overflow-hidden border border-slate-200 cursor-zoom-in"><Image src={validImages[0]} alt="moment" fill className="object-cover" sizes="(max-width: 768px) 80vw, 600px" /></div>;
  if (validImages.length === 4) return <div className="mt-3 grid grid-cols-2 gap-1 max-w-[240px]">{validImages.map((img, i) => <div key={i} className="relative aspect-square bg-slate-100 overflow-hidden cursor-zoom-in"><Image src={img} alt={`img-${i}`} fill className="object-cover hover:scale-110 transition-transform duration-500" sizes="(max-width: 768px) 40vw, 120px"/></div>)}</div>;
  return <div className="mt-3 grid grid-cols-3 gap-1 max-w-[360px]">{validImages.map((img, i) => <div key={i} className="relative aspect-square bg-slate-100 overflow-hidden cursor-zoom-in"><Image src={img} alt={`img-${i}`} fill className="object-cover hover:scale-110 transition-transform duration-500" sizes="(max-width: 768px) 30vw, 120px"/></div>)}</div>;
};
...
export default function LogsPage() {
  ...
  async function handlePublish() {
    ...
    const imagesArray = newImages ? newImages.split(',').map(url => url.trim()).filter(url => isValidUrl(url)) : [];
  ...
  return (
    <div className="relative min-h-screen bg-white text-slate-900 font-sans">
      ...
      {/* ✨ 布局调整 */}
      <main className="w-full lg:ml-72 2xl:ml-80 min-h-screen pb-20 transition-all duration-300">
        <div className="relative h-72 lg:h-96 bg-slate-800 overflow-hidden group">
          <Image src="https://images.unsplash.com/photo-1579546929518-9e396f3cc809?q=80&w=1200" alt="header" fill className="object-cover opacity-90 group-hover:scale-105 transition-transform duration-[20s]" priority />
          <div className="absolute bottom-[-30px] right-6 lg:right-10 flex items-end gap-4 z-10">
            ...
            <div className="w-20 h-20 bg-white p-1 rounded-2xl shadow-xl rotate-3 hover:rotate-0 transition-transform cursor-pointer relative">
               <Image src={user?.user_metadata?.avatar_url || `https://api.dicebear.com/7.x/adventurer/svg?seed=${user?.email || 'milk'}`} alt="avatar" fill className="rounded-xl bg-slate-100 object-cover" />
            </div>
          </div>
        </div>

        {/* ✨ 内容容器居中：max-w-3xl + mx-auto */}
        <div className="max-w-3xl mx-auto pt-24 px-6 lg:px-8">
          {user && (
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-200">
                    ...
                    <button onClick={handlePublish} className="bg-slate-900 text-white px-6 py-2 rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-purple-600 transition-colors flex items-center gap-2"><Send size={12} /> 发布</button>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            {moments.map((moment) => (
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} key={moment.id} className="flex gap-4 border-b border-slate-100 pb-10 last:border-0">
                <div className="w-10 h-10 rounded-lg bg-slate-100 flex-shrink-0 overflow-hidden cursor-pointer hover:opacity-80 relative">
                   <Image src={moment.profiles?.avatar_url || `https://api.dicebear.com/7.x/adventurer/svg?seed=${moment.author_email}`} alt="avatar" fill className="object-cover" />
                </div>
