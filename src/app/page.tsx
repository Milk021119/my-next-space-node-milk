...
import Sidebar from '@/components/Sidebar'; // 假设你已经移到了 layout 目录，如果没有请改回 '@/components/Sidebar'
...
// ✨ 引入我们封装好的用户 Hook (可选，如果你已经创建了的话)
// 如果还没创建，这里下面的代码依然兼容旧的 useState 方式，但我还是建议你用 Hook
...
export default function Page() {
  // 如果你用了 useCurrentUser，可以把下面这三行换成: const { user } = useCurrentUser();
                ...
                <div className="p-6 bg-white/60 backdrop-blur-md rounded-[2rem] border border-white/50 shadow-xl hover:shadow-2xl transition-all duration-500">
                  ...
                  <div className="flex justify-end mt-4">
                    <button 
                      ...
                      className="px-6 py-2 bg-slate-900 text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-transform shadow-lg flex items-center gap-2"
                    >
                      <Send size={12} /> 发布文章
                    </button>
                  </div>
                      ...
                      <div className="flex items-center space-x-3 mb-4 flex-wrap">
                        ...
                        {post.tags?.map(tag => (
                          <Link 
                            ...
                            className="text-[10px] font-bold text-purple-600 bg-purple-50 px-2 py-0.5 rounded-full uppercase cursor-pointer hover:bg-purple-600 hover:text-white transition-colors"
                          >
                             {tag}
                          </Link>
                        ))}
