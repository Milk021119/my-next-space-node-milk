...
export default function ProfilePage() {
  ...
  if (loading) return <div className="min-h-screen bg-slate-50 flex items-center justify-center text-slate-400"><Loader2 className="animate-spin mr-2"/> 加载中...</div>;
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5] font-sans flex">
      <Sidebar />
      {/* ✨ 布局调整 */}
      <main className="flex-1 lg:ml-72 2xl:ml-80 transition-all duration-300">
        <div className="h-64 w-full bg-slate-300 relative overflow-hidden group">
          <Image src={formData.bg_url || profile.bg_url || "https://images.unsplash.com/photo-1579546929518-9e396f3cc809?q=80&w=1200"} alt="background" fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
        ...
        {/* ✨ 内容容器居中：max-w-5xl + mx-auto */}
        <div className="max-w-5xl mx-auto px-6 lg:px-10 relative -mt-20 z-10 pb-20">
          <div className="flex flex-col md:flex-row md:items-end gap-6 mb-8">
            <div className="relative group mx-auto md:mx-0">
              <div className="w-36 h-36 rounded-full border-4 border-white bg-white shadow-2xl overflow-hidden relative">
                <Image src={formData.avatar_url || profile.avatar_url || `https://api.dicebear.com/7.x/adventurer/svg?seed=${profile.id}`} alt="avatar" fill className="object-cover" />
